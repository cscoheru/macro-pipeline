# Claude Code — MiniMax-M3 抽 claim（第 9 批，最后 1 stream）

> **签发**：Cursor（2026-08-26 17:15）  
> **前序**：`reviews/CLAIM_MINIMAX_M3_BATCH8_REPORT_2026-08-26.md` 5/5 analyze PASS  
> **不问用户**；交卷 → `WAIT_CURSOR`
...
这是 streams 队列最后一条。忽略 shorts。禁止 ASR。禁止 DeepSeek。不准写 `data/store.db`。不重跑已抽过的 ID。
...
完成后写 `reviews/CLAIM_MINIMAX_M3_BATCH9_REPORT_2026-08-26.md`。若队列空 → INBOX=`WAIT_USER`「队列空」。
...
HANDOFF 追加。INBOX=`WAIT_CURSOR`（或队列空则 `WAIT_USER`）。commit+push（勿提交 `config/houchen_analyze.env`）。
